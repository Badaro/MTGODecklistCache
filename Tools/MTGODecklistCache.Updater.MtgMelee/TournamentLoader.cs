...
namespace MTGODecklistCache.Updater.MtgMelee
{
    public static class TournamentLoader
    {
        ...
        private static Tuple<Deck, Standing>[] ParseDecks(string url)
        {
            ...
            var standingNode = doc.DocumentNode.SelectNodes("//div[@id='standings-phase-selector-container']").First();
            var activeStandingNode = standingNode.SelectNodes("button[@class='btn btn-primary round-selector']").Last();
            var standingId = activeStandingNode.Attributes["data-id"].Value;
            ...
            do
            {
                ...
                string standingsParameters = _standingsParameters.Replace("{start}", offset.ToString());
                string standingsUrl = _standingsPage.Replace("{standingId}", standingId);

                string json = Encoding.UTF8.GetString(new WebClient().UploadValues(standingsUrl, "POST", HttpUtility.ParseQueryString(standingsParameters)));
                var standings = JsonConvert.DeserializeObject<dynamic>(json);

                foreach (var player in standings.data)
                {
                    hasData = true;
                    string playerName = player.Name;
                    string playerRecord = player.Record;
                    ...
                    foreach (var card in cardList)
                    {
                        ...
                        {
                            if (insideCompanion) continue; // Companion is listed separately *and* in the sideboard as well
